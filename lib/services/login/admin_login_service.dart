...
// 관리자 로그인 로직 수행 함수
Future<void> adminLoginService(String loginId) async {
  final connectivityController = Get.put(ConnectivityController());
  final LoginController loginController = Get.put(LoginController());

  if (connectivityController.isConnected.value) {
    // 사용자 로그인 정보(아이디,비밀번호)를 기기에 저장
    String url = dotenv.get('LOGIN_PASS_URL');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (resultValue == "0000") {
          ...
          loginDataController.setSnamesList([loginData]);
    ...
    // 응답을 받지 못했을 때
      ...
      if (id == "") {
        failDialog1('로그인 실패', '아이디를 입력해주세요.');
      } else {
        await failDialog1('로그인 실패', '아이디를 확인해 주세요.');
        // 아이디, 비밀번호 입력 초기화
