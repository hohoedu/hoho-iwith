...
// 유아 월간 학습내용 가져오기(한자)
Future<void> haniMonthlyReportService(String stuId, String ym, String type) async {
  final haniMonthlyReport = Get.put(HaniMonthlyReportDataController());
  final classInfo = Get.find<ClassInfoDataController>().classInfoDataList;

  String url = dotenv.get('HANI_MONTHLY_REPORT_URL');
  try {
    ...
    if (response.statusCode == 200) {
      ...
      // 응답 결과가 있는 경우
      if (resultValue == "0000") {
        final List<HaniMonthlyReportData> monthlyReportDataList =
            (resultList['data'] as List).map((json) => HaniMonthlyReportData.fromJson(json)).toList();
        haniMonthlyReport.setHaniMonthlyReportDataList(monthlyReportDataList);

        await classInfoService(stuId);
      }
      // 응답 데이터가 오류일 때("9999": 오류)
      else {
        haniMonthlyReport.setHaniMonthlyReportDataList([]);
        await classInfoService(stuId);
      }
    }
  }
